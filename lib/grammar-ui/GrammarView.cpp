...
#include "../grammar-core/GrammarInducer.h"
...
// ============================================================
// Toolbar section — registered with EditorUI at startup
// Draws grammar action buttons inside the main toolbar when
// EditorMode::GRAPH_GRAMMAR is active.
// ============================================================

void GrammarView::drawToolbar(Scene& scene, MeshLibrary& lib)
{
    // Status indicator — compact, left of buttons
    if (m_animating) {
        const auto& st = m_grammar.state();
        float progress = (st.maxAttempt > 0)
            ? (float)st.attempt / (float)st.maxAttempt : 0.f;
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4{1.f, 0.8f, 0.2f, 1.f});
        ImGui::Text("Gen... %d%%", (int)(progress * 100));
        ImGui::PopStyleColor();
    } else {
        const auto& st = m_grammar.state();
        if (st.success)
            ImGui::TextColored({0.3f, 1.f, 0.3f, 1.f}, "OK (%d pcs)", (int)m_grammar.placed.size());
        else if (st.failed)
            ImGui::TextColored({1.f, 0.35f, 0.35f, 1.f}, "Failed");
        else
    ...
    // Generate button
    ImGui::PushStyleColor(ImGuiCol_Button,        {0.22f, 0.45f, 0.22f, 1.f});
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, {0.28f, 0.62f, 0.28f, 1.f});
    if (ImGui::Button(" Generate ") && !m_animating) {
        m_grammar.seed += 100;
        startGenerate(scene, lib);
    }
    ImGui::PopStyleColor(2);
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Generate new layout (Space)");
    ...
    if (ImGui::Button(" Reset ") && !m_animating) {
        m_grammar.seed = 42;
        startGenerate(scene, lib);
    }
    ImGui::PopStyleColor(2);
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("Reset seed to 42 and regenerate");

    ImGui::SameLine();

    // Step Mode toggle
    bool stepActive = m_stepMode;
    if (stepActive) ImGui::PushStyleColor(ImGuiCol_Button, {0.50f, 0.30f, 0.55f, 1.f});
    else            ImGui::PushStyleColor(ImGuiCol_Button, {0.28f, 0.32f, 0.42f, 1.f});
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, {0.65f, 0.42f, 0.72f, 1.f});
    if (ImGui::Button(" Step "))
        m_stepMode = !m_stepMode;
    ImGui::PopStyleColor(2);
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Step mode: advance one attempt per frame");
}
